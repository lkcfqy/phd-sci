"""Run post-reveal topology-disjoint matched-session calibration on the PMSG."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--protocol",
        type=Path,
        default=Path("docs/paper3_post_reveal_matched_calibration_protocol.md"),
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
...
def main() -> None:
    ...
    fault_records = residuals[~residuals["is_healthy_file"].astype(bool)].copy()
    cases = pmsg_topology_assignments(residuals)
    fault_records = fault_records.merge(
        cases[
            [
                "fault_family",
                "terminal_a",
    ...
    for fold in range(3):
        ...
        fit_bucket = (fold + 2) % 3
        fit = fault_records[
            fault_records["topology_bucket"].eq(fit_bucket)
            & fault_records["evaluation_role"].eq("pre_fault_test")
        ]
        calibration = fault_records[
            fault_records["topology_bucket"].eq(calibration_bucket)
            & fault_records["evaluation_role"].eq("pre_fault_test")
        ]
        test = fault_records[
            fault_records["topology_bucket"].eq(test_bucket)
            & fault_records["evaluation_role"].isin(["pre_fault_test", "fault_test"])
        ].copy()
        ...
        fold_rows.append(
            {
                ...
                "fit_healthy_records": len(fit),
                "calibration_healthy_records": len(calibration),
                "test_records": len(pre),
                "threshold": threshold,
                "pre_false_alarms": int(pre["alarm"].sum()),
                "pre_session_far": float(pre["alarm"].mean()),
                "fault_records_detected": int(detection.sum()),
                "fault_record_detection": float(detection.mean()),
            }
        )
    windows = pd.concat(fold_windows, ignore_index=True)
    if windows["record_id"].nunique() != 216:
        raise AssertionError("Every PMSG fault record must be tested exactly once")
    per_record_metadata = [
        "record_id",
        "fault_family",
        "speed_rpm",
    ...
    pre_summary = pre.groupby("record_id").agg(
        pre_score=("score", "max"), pre_session_false_alarm=("alarm", "any")
    )
    fault_summary = active.groupby("record_id").agg(
        fault_score=("score", "max"), fault_record_detection=("alarm", "any")
    )
    ...
    summary = {
        "analysis_status": "post-reveal topology-disjoint internal validation",
        "method": "matched_session_anchor_ledoit_wolf",
        "folds": 3,
        "records": len(records),
    ...
    if not math.isfinite(sum(row["threshold"] for row in fold_rows)):
        raise FloatingPointError("A topology-fold threshold is not finite")
    ...
    pd.DataFrame(fold_rows).to_csv(args.output_dir / "per_fold_summary.csv", index=False)
